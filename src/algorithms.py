...
class algorithms:
    ...
    def trainrandomforest(self,dataset_name,predictors,target):
        ...
        dataset_files=datasetObj.get_datasetsFirstFilenametest(dataset_name)
        bashCommand="$HADOOP_PREFIX/bin/hadoop jar SMAHOUT_HOME/core/target/mahout-core-1.0-SNAPSHOT-job.jar org.apache.mahout.classifier.df.tools.Describe -p /data/"+dataset_name+"/train/"+dataset_files[0]+" -f /data/"+dataset_name+"/train/"+dataset_files[0]+".info -d "+randomForestLabelString
        process = subprocess.Popen(bashCommand, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
        output, error = process.communicate()
