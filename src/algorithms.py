...
class algorithms:
    ...
    def trainlogistic(self,dataset_name,predictors,target):
        ...
        for i in predictors:
            count+=1
            str_predictor+=i+" "
        bashCommand="$MAHOUT_HOME/bin/mahout trainlogistic  --input ~/code/bdmaas/data/"+dataset_name+"/data  --output ~/code/bdmaas/data/"+dataset_name+"/model  --target "+target+"   --categories "+count+"  --predictors "+str_predictor+"   --types word  --features 20   --passes 100 --rate 50"
    ...
    def runlogistic(self,dataset_name,predictors,target):
        str_predictor=""
        count=0
        for i in predictors:
            count+=1
